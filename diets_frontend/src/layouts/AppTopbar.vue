...
const userInfo = computed<UserInfo>(() => {
    return (useCookie('authUser').value as UserInfo) || {}
})

// Route handling
const currentRoute = computed(() => route.fullPath.split('/').filter(Boolean))
const isDoctorsPage = computed(() => currentRoute.value[0] === 'doctor')
...
<template>
    <div class="layout-topbar">
        ...
        <div class="layout-topbar-logo-container">
            <button v-if="!isDoctorsPage" class="layout-menu-button layout-topbar-action" @click="onMenuToggle">
        ...
        <div class="layout-topbar-actions">
            ...
            <!-- User Profile -->
            <div class="flex items-center gap-2 hover:cursor-pointer" @click="toggleUserInfo">
                <Icon name="material-symbols:account-circle" size="2rem" /> 
                <span class="text-sm">
                    {{ userInfo.firstname }} {{ userInfo.lastname }}
                    <p v-if="userInfo.postitle" class="text-xs italic">
                        {{ userInfo.postitle }}
                    </p>
                </span>
                <Icon name="material-symbols:arrow-drop-down-rounded" size="2rem" />
            </div>
            
            <!-- User Dropdown -->
            <Popover ref="showUserInfo" class="w-38">
                <div :class="['logout_button', { 'opacity-75': loading }]" @click="!loading && handleLogout()">
                    <Icon name="material-symbols:logout-rounded" size="1.5rem" />
                    <span class="text-sm">
        ...
        <Dialog v-model:visible="patientSearchModal"  header="Patient Search" :draggable="false" modal dismissableMask class="w-3/4">
            <SearchPatients v-if="patientSearchModal" @close="patientSearchModal = false"/>
        </Dialog>
    </div>
</template>
...
<style scoped>
.logout_button {
  @apply w-full flex flex-row gap-2 items-center px-4 py-2 rounded-lg;
}

.logout_button:hover {
  @apply cursor-pointer bg-gray-100 dark:bg-gray-700;
}
</style>